...
class SalesmanController extends BaseController
{
    public function index(){
        // 获取公会列表
        $guild_list = GuildModel::where(['status'=>1])->select();
    ...
    public function getlist(){
        ...
        $where['is_salesman'] = 1;
        $guildid > 0 && $where['guildid'] = $guildid;
        if ($searchid){
            $where["id"] = $searchid;
        }
        if ($searchisonline != ''){
    ...
    public function invitList(){
        ...
        $where['is_salesman'] = 0;
        $agent_id > 0 && $where['agentid'] = $agent_id;
        $agent_ids = UserModel::where(['is_salesman'=>1])->column('id');
        $user_list = UserModel::where($where)
            ->whereIn('agentid', $agent_ids)
            ->with(['profile'])
    ...
    public function recharge(){
        // 获取业务员列表
        $user_list = UserModel::where(['is_salesman'=>1])->select();
