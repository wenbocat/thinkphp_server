...
{
    protected $NeedLogin = ['getAliPayOrder','getWxPayOrder'];
    ...
    public function getAliPayOrder(){
        ...
        $order = new OrderModel(['uid'=>$this->userinfo->id,'vip_level'=>$item->level,'type'=>1,'order_no'=>$order_no,'amount'=>$item->price,'gold'=>$item->gold,'pay_channel'=>2,'create_time'=>nowFormat()]);
        if ($order->save()) {
