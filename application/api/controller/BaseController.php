...
class BaseController extends Controller
{
    ...
    public function check_params(){
        if (array_key_exists(Request::action(),$this->rules)){
            ...
            if (!$validater->check(Request::param())) {
                $json = json_encode(array("status"=>"1","msg"=>$validater->getError()));
                $response = Response::create($json);
                throw new HttpResponseException($response);
            }
        }
    ...
    static function bulidDataFail($data=['st'=>0], $message="操作失败，请稍后再试"){
    ...
    static function bulidSuccess($data=['st'=>0],$msg=""){
    ...
    static protected function getOptions(){
        ...
        //$options->alipayCertPath = Env::get('root_path').'cert/alipay/alipayCertPublicKey_RSA2.crt';//'<-- 请填写您的支付宝公钥证书文件路径，例如：/foo/alipayCertPublicKey_RSA2.crt -->';
        //$options->alipayRootCertPath = Env::get('root_path').'cert/alipay/alipayRootCert.crt'; //'<-- 请填写您的支付宝根证书文件路径，例如：/foo/alipayRootCert.crt" -->';
        //$options->merchantCertPath = Env::get('root_path').'cert/alipay/appCertPublicKey_2021003148612034.crt';//'<-- 请填写您的应用公钥证书文件路径，例如：/foo/appCertPublicKey_2019051064521003.crt -->';
    ...
    /* 生成订单号 */
    static protected function createOrderNo($uid,$prefix=''){
        $orderid = $prefix . $uid.date('YmdHis').rand(1000,9999);
        return $orderid;
    }
