...
{
    protected $NeedLogin = ['getAttentList', 'unlockMoment', 'likeMoment', 'collectMoment', 'likeComment', 'publish', 'getTempKeysForCos', 'getSignForVod', 'publishComment', 'getCollection','reoprt'];
    ...
    public function search(){
        ...
        $moments = MomentModel::where(['status'=>1, 'is_secret'=>0])->whereRaw($whereRaw)->with(['user'])->order(['recommend'=>'desc','create_time'=>'desc'])->limit(($page-1)*$size,$size)->select();
    ...
    public function listInTopic(){
        ...
        if ($type == 0){
            $order = ['recommend'=>'desc','comment_count'=>'desc','create_time'=>'desc'];
        }

        $moments = MomentModel::where(['status'=>1, 'is_secret'=>0, 'topic'=>$topic])->with(['user'])->order($order)->limit(($page-1)*$size,$size)->select();
    ...
    public function publish(){
        ...
        $moment = new MomentModel(['uid'=>$this->userinfo->id, 'type'=>$type, 'title'=>$title, 'image_url'=>$image_url, 'video_url'=>$video_url, 'blur_image_url'=>$blur_image_url, 'single_display_type'=>$single_display_type, 'unlock_price'=>$unlock_price, 'topic'=>$topic, 'status'=>$status, 'create_time'=>nowFormat()]);
        if ($moment->save()){
            if ($topic){
                //话题参与人次+1
    ...
    public function getComments(){
        ...
        $comments = MomentCommentModel::where(['momentid'=>$momentid, 'rootid'=>0])->where('id','<',$lastid)->with(['user'])->order('create_time','desc')->limit(0,$size)->select();
    ...
    public function reoprt(){
        $model = new MomentReportModel(Request::param());
        $model->momentid = Request::param('relateid');
