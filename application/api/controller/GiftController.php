...
use app\common\model\GiftLogAdminModel;
...
class GiftController extends BaseController
{
    ...
    public function sendGift(){
        ...
        $pkid = Request::param('pkid') ?? 0;
        $pkto = Request::param('pkto') ?? 0; //1-home 2-away
...
//            //金币钻石结算
            ...
            if ($liveid){
                $live_profit_update = Db::table('db_live')->where('liveid',$liveid)->inc(['profit'=>$gold_cost,'hot'=>$gold_cost*100])->update();
            }

            $pk_update = 1;
            if ($pkid && $pkto){
                //更新pk得分
                $pk_score_prop = $pkto == 1 ? 'home_score' : 'away_score';
                $pk_update = Db::table('db_live_pk')->where('id',$pkid)->inc($pk_score_prop,$gold_cost)->update();
            }

            if($anchor->save() && $anchor_profit->save() && $user->save() && $user_profit->save() && $intimacy->save() && $userConsume->save() && $live_profit_update && $pk_update){
                Db::commit();
                ...
                if ($liveid){
                    if ($pkid && $pkto) {
                        //推送pk信息
                        $pkinfo = LivePkModel::where(['id' => $pkid])->find();
                        $im_ext_pk['notify'] = ['type' => 'RoomNotifyTypePkScoreChange', 'pkinfo' => $pkinfo];
                        $elem_pk = TXService::buildCustomElem('RoomNotification', $im_ext_pk);
                        TXService::sendChatRoomMsg(getAnchorRoomID($pkinfo->home_anchorid), $elem_pk);
                        TXService::sendChatRoomMsg(getAnchorRoomID($pkinfo->away_anchorid), $elem_pk);
                    }

                    //通过IM推送到直播间
                    $gift->sender = ["id" => $user->id,'nick_name' => $user->nick_name,'user_level' => $user->user_level,'avatar' => $user->avatar];
                    $gift->count = $count;
                    $im_ext['gift'] = $gift;
                    $im_ext['live']['profit'] = $gold_cost;
                    ...
                    if ($gift->use_type == 1){
                        ...
                        for ($i = 0; $i<count($uidArr);){
                            ...
                            $i += 500;
                            $elem = TXService::buildCustomElem('BroadcastStreamer',['streamer'=>['user'=>$user->visible(['nick_name','avatar','id']),'gift'=>$gift,'anchor'=>['nick_name'=>$anchor->nick_name],'type'=>2]],'全频道广播','');
                            TXService::sendBroadcast($pushids,$elem);
                        }
                    }
                }
                exit();
