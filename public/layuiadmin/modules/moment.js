layui.define(['table', 'form', 'upload', 'admin'], function(exports) {
    ...
    table.render({
        elem: '#LAY-moment-manage'
        , url: '/admin/moment/getlist'
        , cols: [[
            ...
            , {field: '', title: '图片', width: 80, templet: function (d) {
                    if (d.type != 1 && d.image_url.length>0){
                        var images = d.image_url.split(",");
                        return '<img style="display: inline-block; width: auto; height: 100%;" src= "'+ images[0] +'" layadmin-event="photosPreview" value="'+d.image_url+'">'
                    }else {
                        return "未上传";
                    }
                }}
            , {field: '', title: '视频', width: 80, templet: function (d) {
                    if (d.type == 3 && d.video_url.length>0){
                        return '<img style="display: inline-block; width: auto; height: 100%;" src= "'+ d.image_url +'" layadmin-event="videoPreview" value="'+d.video_url+'">'
                    }else {
                        return "未上传";
                    }
                }}
            , {field: 'unlock_price', title: '解锁价格', width: 100}
            , {field: 'recommend', title: '置顶', width: 100, templet: function (d) {
                    if (d.recommend == 1){
                        return '<span class="layui-badge layui-bg-green">置顶</span>';
                    }
                    return '<span class="layui-badge layui-bg-gray">未置顶</span>';
                }}
            , {title: '操作', width: 150, align: 'center', fixed: 'right', toolbar: '#table-momentlist-operation'}
        ]]
        , page: true
        , limit: 20
        , height: 'full-140'
    ...
    //监听工具条
    table.on('tool(LAY-moment-manage)', function(obj){
        if(obj.event === 'del'){
            var title = obj.data.status == 1?"下架":"上架";
            layer.confirm('确定'+title+'动态？', function(index){
                var status = obj.data.status == 1?3:1;
                admin.req({
                    url: '/admin/moment/del_post'
            ...
            var index = layer.open({
                type: 2
                ,title: '动态'
                ,content: '/admin/moment/edit?id='+obj.data.id
                ,maxmin: true
                ,area: ['600px', '550px']
            });
            layer.full(index);
    ...
    form.on('submit(moment-add)', function(obj){
        var data = obj.field;
        //提交
        admin.req({
            url: '/admin/moment/add_post'
            ,data: data
    ...
    function getVodSignature() {
        return axios.post('/admin/moment/signForVod').then(function (response) {
    ...
    table.render({
        elem: '#LAY-moment-report-manage'
        , url: '/admin/moment/getReportList'
        , cols: [[
            {field: 'id', width: 100, title: 'ID', sort: true, fixed: 'left'}
            , {field: '', title: '举报用户', width: 200, templet: function (d) {
                    return d.user.nick_name + "("+ d.uid+")";
                }}
            , {field: '', title: '动态标题', width: 200, templet: function (d) {
                    return d.moment.title;
                }}
    ...
    //监听工具条
    table.on('tool(LAY-moment-report-manage)', function(obj){
        console.log(obj);
        if(obj.event === 'view'){
            var index = layer.open({
                type: 2
                ,title: '动态'
                ,content: '/admin/moment/edit?id='+obj.data.momentid
                ,maxmin: true
                ,area: ['600px', '550px']
            });
            layer.full(index);
